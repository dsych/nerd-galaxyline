...
local function get_function_info()
	...
	elseif require("lsp-status") then
	...
	GitBranch = {
		provider = function()
			return require("galaxyline.condition").check_git_workspace()
					and require("galaxyline.provider_vcs").get_git_branch()
				or "Not a Repo"
		end,
...
require("galaxyline").section.short_line_left = {
	{
		ShortLineLeftBufferType = {
			...
			provider = function()
				local name = buffer_type_map[vim.bo.filetype] or " Editor"
	...
	},
	{
		ShortLineLeftWindowNumber = {
            ...
            condition = function()
                return not buffer_type_map[vim.bo.filetype]
            end,
			separator = "",
